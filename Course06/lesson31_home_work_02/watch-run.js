...
const psList = require('ps-list').default; // <- вот тут важно .default
...
const PROCESS_TAG = "MY_UNIQUE_APP_TAG"; // любое уникальное имя


let javaProcess = null;
let restartTimeout = null;

async function killJavaByTag(tag) {
    const processes = await psList();
//    const filePath = path.resolve(__dirname, 'processes.json');
//    fs.writeFileSync(filePath, JSON.stringify(processes, null, 2), 'utf8');


    // here only OUR PROCESS
    const javaProcesses = processes.filter(p =>
        p.name.toLowerCase() === 'java.exe' // &&
        // DOES NOT WORK!!!
        // p.cmd && p.cmd.includes(tag) // <- проверяем, что cmd существует
    );
    console.log("KILLING PROCESSES: ", javaProcesses);
    if (javaProcesses.length === 0) {
        console.log(`No Java process found with tag: ${tag}`);
        return;
    }

    for (const proc of javaProcesses) {
        console.log(`Killing process PID ${proc.pid} with tag ${tag}`);
        try {
            execSync(`taskkill /PID ${proc.pid} /T /F`);
        } catch (err) {
            console.log(`Failed to kill PID ${proc.pid}: ${err.message}`);
        }
    }
}
...
function startJava() {
  ...
  */
    javaProcess = spawn(MAVEN_CMD, ["compile exec:java", "-Dexec.mainClass=Main", `-Dexec.args=${PROCESS_TAG}`], {
        stdio: "inherit",
        cwd: path.resolve(__dirname),
        shell: true
    });
...
watcher.on("all", (event, filePath) => {
  ...
  restartTimeout = setTimeout(() => {
    ...
    //killPrevious();
    // killJavaByTag("MY_UNIQUE_APP_TAG");

    // Использование внутри async функции
    (async () => {
        await killJavaByTag('MY_UNIQUE_APP_TAG');
    })();
