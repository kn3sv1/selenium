...
//            String closeTag = "</span>";
            ...
            for (int i = 0; i < response.length(); i++) {
                if (Character.isDigit(response.charAt(i))) {
                    ...
                    // this is current number variable where we store current digit
                    String digit = "";
                    ...
                    // if the digit is the same as previous digit HERE we should continue iteration and not execute
                    // code bellow that changes index for color.
                    boolean isEqual = digit.equals(previousNumber);
                    // version 4: we take previous stored current index
                    if (isEqual) {
                    ...
                    // always change previous to current digit - doesn't mater if it's equal or not.
                    // Extra variable we use to compare before I change variable previousNumber
